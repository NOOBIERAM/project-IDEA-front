...
import { Clock, Flame, ExternalLink, Trash2, RotateCcwSquare } from "lucide-react"
...
const TrashPage = () => {
    ...
    const fetchTrashIdeas = async () => {
        try {
            setIsLoading(true);
            const ideas = await getProjectsInTrash();
            setIdeas(ideas);
        } catch (error) {
            console.error("-- Error while fetching trash ideas -- ", error)
        } finally {
            setIsLoading(false);
        }
    }
    ...
    const handleSearch = async (query: string) => {
        try {
            setIsLoading(true);
            if (query) {
                const data = await searchInTrash(query)
                setIdeas(data);
            }
            else {
                fetchTrashIdeas()
            ...
            console.error("-- Error while searching in trash -- ", error)
        }
        finally {
            setIsLoading(false);
        }
        ...
        <>
            ...
            <section className="max-w-full w-full h-full pt-10 relative">
                <div className="mb-10 px-10 flex justify-between items-center">
                    <h1 className="font-bold text-3xl ">Corbeille</h1>
                            ...
                            <div key={index} className={`group transition-all duration-300 ease-out hover:-translate-y-1 bg-white border border-gray-400 shadow-lg hover:${getColors(idea.level).shadow} rounded-xl p-3 max-lg:w-full`}>
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className="flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100  max-lg:opacity-100 transition-opacity">
                                        ...
                                        <button className="relative group/item flex items-center border rounded-full px-2 py-1  text-gray-500 hover:text-gray-800" onClick={() => restoreFromTrash(idea.projectId)}>
                ...
                {
                    selectedIdea && <DetailModal idea={selectedIdea} onDelete={() => { handleDelete(selectedIdea.projectId) }} onRestore={()=>{handleRestoreFromTrash(selectedIdea.projectId)}} onCancel={() => {
